    ...
    if (move_uploaded_file($file['tmp_name'], $newFilePath)) {
        ...
        if ($storage === 'local') {
            handleLocalStorage($finalFilePath, $newFilePath, $uploadDir, filesize($finalFilePath), 
                $dimensions['width'], $dimensions['height'], $randomFileName, $user_id, $upload_ip, $category);
 ...
 * 插入图片记录到数据库
 */
function insertImageRecord($fileUrl, $path, $storage, $size, $upload_ip, $user_id, $category = '默认') {
    global $mysqli;

    $stmt = $mysqli->prepare("INSERT INTO images (url, path, storage, size, category, upload_ip, user_id) VALUES (?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sssdssd", $fileUrl, $path, $storage, $size, $category, $upload_ip, $user_id);
    $stmt->execute();
    $stmt->close();
}
