...
// 获取总记录数
$count_sql = "SELECT COUNT(id) as total FROM images" . $where_clause;
if (!empty($category)) {
    ...
    $total_rows = (int)($mysqli->query($count_sql)->fetch_assoc()['total'] ?? 0);
}
...
// 构建SQL查询
$sql = "SELECT * FROM images" . $where_clause . " ORDER BY id DESC LIMIT ? OFFSET ?";
$params = $count_params;
$params[] = $items_per_page;
$params[] = ($current_page - 1) * $items_per_page;
...
if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && 
    ...
    if (isset($_GET['action']) && $_GET['action'] === 'get_categories') {
        $categories_result = $mysqli->query("SELECT DISTINCT category FROM images WHERE category IS NOT NULL AND category != '' ORDER BY category");
        $categories = [];
...
<body>
    ...
    <div class="rightside">
        ...
        <a class="category-link" title="分类管理"><img src="/static/images/svg/category.svg" alt="分类管理"></a>
    ...
    <div id="category-modal" class="modal">
        <div class="modal-content">
            <div class="settings-container">
                <button class="close-modal" onclick="document.getElementById('category-modal').classList.remove('show')">
                    <img src="/static/images/svg/xmark.svg" alt="关闭">
                </button>
                <h2>分类管理</h2>
    ...
    foreach ($images as $image): ?>
        <div class="gallery-item" id="image-<?= htmlspecialchars($image['id']) ?>">
            ...
            <div class="action-buttons">
                ...
                <button class="download-btn" data-url="<?= htmlspecialchars($image['url']) ?>">
                    <img src="/static/images/svg/download.svg" alt="Download" />
                </button>
            ...
            <div class="image-info">
                <p class="info-p">分类: <span><?= htmlspecialchars($image['category'] ?? '默认') ?></span></p>
